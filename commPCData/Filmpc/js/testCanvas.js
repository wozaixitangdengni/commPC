layui.define(function (exports) {
    exports("testCanvas", function ($) {
        ...
        var obj ={
            ...
            posX_cope:0,
            posY_cope:0,
            seat_posX_posY:[],//保存座位坐标
            selectedSeat:[],//保存选中座位
            max_buy:max_buy,//限购数量
            is_msg:is_msg,//是否填写资料 1：填写资料，0：不填写
            order_money:0,
            init:function(){
                _this = this;
                //画布容器
                ...
                $(".select-area .select-area-main .select-area-area .selected_seat .seat_list").on("click","li div i",function(){
                    var item_i = $(this).parent().parent().index();
                    $.each(_this.selectedSeat,function(i,item){
                        if(item_i == i){
                            item.isSelected = !item.isSelected;
                            _this.selectedSeat.splice(i,1);
                            _this.drawCanvasBuffer("CLICK");
                            _this.showSelectedSeat();
                            return false;
                        }
                    });
                });
                ...
                $(".select-area .select-area-main .select-area-area .selected_seat .submit_order").on("click",_this.goSubmit);
                        ...
                        if(type == "CLICK"){
                            ...
                            if(selected_seat.status != 0){
                                ...
                                if(selected_seat.isSelected){
                                    if(_this.selectedSeat.length >= max_buy){
                                        selected_seat.isSelected = !selected_seat.isSelected;
                                        layer.msg('最多购买'+max_buy+'张', {icon: 2});
                                        return false;
                                    }
            ...
            showSelectedSeat:function(){
                $(".select-area .select-area-main .select-area-area .selected_seat .seat_list").html('');
                var elem = null;
                if(!_this.selectedSeat.length){
                    $(".select-area .select-area-main .select-area-area .set_scale_box").css("right","0");
                    $(".select-area .select-area-main .select-area-area .selected_seat").hide();
                }else{
                    $(".select-area .select-area-main .select-area-area .set_scale_box").css("right",190);
                    $.each(_this.selectedSeat,function(i,item){
                        elem = $(`<li>
                                <div>
                                    <img src="${root}/images/seat.png" alt="" style="background-color: red"/>
                                    <span>￥${item.price}</span>
                                    <i></i>
                                </div>
                                <p><span>${item.hall_title}</span>${item.seat_title}</p>
                            </li>`);
                        elem.find("img").css("backgroundColor",item.color);
                        $(".select-area .select-area-main .select-area-area .selected_seat .seat_list").append(elem);
                    });
            ...
            getOrderPrice:function(){
                ...
                $(".select-area .select-area-main .select-area-area .selected_seat .submit_order span").text(money);
            },
            goSubmit:function(){
                var submitObj = {date1:_date,times:_times,hall_id:hall_id,film_id:film_id,seat:[]};
                $.each(_this.selectedSeat,function(i,item){
                    submitObj.seat.push(item.seat);
                });
                $(".top-msg").show();
                $.post(tmp_url,submitObj,function(res){
                    console.log(res);
                    if(res.status == 1){
                        setTimeout(function(){
                            if(is_msg == 1){
                                location.href = seat_info_url;
                            }else if(is_msg == 0){
                                location.href = do_order_url;
                            }
                            $(".top-msg").hide();
                        },1000)
