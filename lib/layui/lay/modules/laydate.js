...
!function () {
    ...
    //添加自定义日期规则
    Class.prototype.dateRule = function (el, option) {
        //year: 2018, month: 0, date: 31
        //console.log(option);
        var date_status, info, option = {};

        if (date_rule_json) {
            date_status = date_rule_json.date_status;
            if (date_status == 0) {
                info = date_rule_json.info;
                //dateDingZhi(el,option,info);
                lay.each(el, function (index, item) {
                    option.year = lay(item).attr("lay-ymd").split('-')[0];
                    option.month = lay(item).attr("lay-ymd").split('-')[1];
                    option.date = lay(item).attr("lay-ymd").split('-')[2];
                    dateDingZhi(item, option, info);
                });
                //option = {year: 2018, month: 1, date: 31};
                //dateDingZhi(el,option,info);
                dateWeek(el,option,info);
    ...
    //定制日期
    function dateDingZhi(el, option, info) {
        var status = null, month_info, date_info, month;
        for (var yearI = 0; yearI < info.length; yearI++) {
            if (info[yearI].year == option.year) {
                status = true;
                month_info = info[yearI].info;
                for (var monthI = 0; monthI < month_info.length; monthI++) {
                    status = true;
                    month = option.month;
                    if (month_info[monthI].month == month) {
                        status = true;
                        date_info = month_info[monthI].date;
                        for (var dateI = 0; dateI < date_info.length; dateI++) {
                            //console.log(info[yearI].year,month,date_info[dateI].date);
                            //console.log(info[yearI].year == option.year,month_info[monthI].month == month,date_info[dateI].date == option.date);
                            //console.log(info[yearI].year , option.year,month_info[monthI].month , month,date_info[dateI].date,option.date);
                            if (!(info[yearI].year == option.year) && !(month_info[monthI].month == month) && !(date_info[dateI].date == option.date)) {

                            } else {
                                //lay(el).addClass("laydate-disabled");
                            }
                        }
                    }
                }
    ...
    //周循环
    function dateWeek(el, option, info) {
        var status = null, month_info, date_info, month;
        for (var yearI = 0; yearI < info.length; yearI++) {
            month_info = info[yearI].info;
            for (var monthI = 0; monthI < month_info.length; monthI++) {
                ...
                for (var dateI = 0; dateI < date_info.length; dateI++) {
                    ...
                    lay.each(el,function(index,item){
                        ...
                        if(info[yearI].year == ymd[0] && month_info[monthI].month == ymd[1] && date_info[dateI].date == ymd[2]){
                            //console.log(item);
                            lay(item).removeClass("laydate-disabled");
                        }else{
                            //console.log(11);
